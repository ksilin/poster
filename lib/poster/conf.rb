...
class Conf
  ...
  def initialize(config_file = CONFIG_FILE)
    ...
    puts "found config in home dir:  #{File.exist? user_config} : #{user_config}"
    folder_config = File.join(Dir.pwd, CONFIG_FILENAME)
    puts "found config in working dir:  #{File.exist? folder_config} : #{folder_config}"

    if File.exist? config_file
      @config_options = YAML.load_file(config_file)
      $stderr.puts "loaded #{config_file}"
